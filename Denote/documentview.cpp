...
DocumentView::DocumentView() : SubWindow(){

}

DocumentView::DocumentView(Document *doc) : SubWindow(){
    this->doc = doc;
}
...
void DocumentView::paintEvent(QPaintEvent *event){
    ...
    p.fillRect(rect(), "red");
    p.save();
    extern void text(QPainter *, int, int, const QColor &);
    text(&p, width(), height(), "red");
    p.restore();
}


void text(QPainter *painter, int w, int h, const QColor &color)
{
    QPainterPath path;
